...
| **Bookmarks**                    | Save or remove bookmarked story IDs; list bookmarked items in a separate tab.                                             | **@reduxjs/toolkit** + **react-redux** (bookmark state), **@react-native-async-storage/async-storage** (persisted ID list)                                                                                      |
| **Cached articles**              | After a successful fetch, caches the story batch to disk; on network failure, can show the last cached list as stale.     | **@react-native-async-storage/async-storage** (JSON cache), **@reduxjs/toolkit** (`loadArticles` fallback)                                                                                                    |
...
| **Light / dark theme**           | Follows system appearance or a stored preference (light / dark / system).                                                 | **react-native** `useColorScheme`, **@react-native-async-storage/async-storage** (persisted preference), **@react-navigation/native** themes for navigation chrome                                            |
